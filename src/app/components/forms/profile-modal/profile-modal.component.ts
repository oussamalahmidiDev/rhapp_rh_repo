...
import {
  ...
  ModifyProfile,
  UnsetPhoto,
} from "src/app/actions/profile.action";
...
export class ProfileModalComponent implements OnInit {
  ...
  closeModal() {
    console.log(
      ...
      this.compteForm.touched,
      this.uploading,
      this.profileFormSubmitted,
      this.compteFormSubmitted
    );

    if (
      (this.profileForm.touched && !this.profileFormSubmitted) ||
      (this.compteForm.touched && !this.compteFormSubmitted) ||
      this.uploading
    ) {
      if (
        confirm(
          "Vous n'avez pas sauvegardé les modifications. Voulez-vous continuer ?"
        )
      ) {
  ...
  handlePhotoUpload($event) {
    ...
    //   }
    // )
    this.userService.uploadAvatar(image).subscribe(
      (data: HttpEvent<any>) => {
        console.log(data);
  ...
  onSubmitProfileForm() {
    ...
    this.store
      .dispatch(new ModifyProfile(this.profileForm.value))
      .subscribe(() => {
